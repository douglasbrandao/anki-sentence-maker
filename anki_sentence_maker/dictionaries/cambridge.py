...
class Cambridge(Base):
    ...
    def scrape(self):
        ...
        definitions: List[str] = [s.text.strip().replace(':', '') for s in soup.find_all('div', class_='def ddef_d db')]
        examples: List[str] = [s.text for s in soup.find_all('div', class_='examp dexamp')]

        dataset_examples = soup.find('div', attrs={'id': 'dataset-example'})

        if dataset_examples:
            examples: List[str] = [s.text.strip() for s in soup.find_all('span', class_='deg')]
